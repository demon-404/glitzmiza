#!/usr/bin/env node
// Simple smoke test for backend endpoints using https module (CommonJS)
...
function headUrl(path) {
  return new Promise((resolve, reject) => {
    try {
      const url = new URL(path, apiBase)
      const req = request(url, { method: 'HEAD' }, (res) => resolve({ status: res.statusCode }))
      req.on('error', (err) => reject(err))
      req.end()
    } catch (err) { reject(err) }
  })
...
;(async () => {
  ...
  } catch (err) { console.error('HEALTH: FAIL', err.message); console.error(err.stack); return process.exit(2) }
  ...
  } catch (err) { console.error('PRODUCTS: FAIL', err.message); console.error(err.stack); return process.exit(2) }

  if (Array.isArray(products) && products.length > 0) {
    const id = products[0]._id
    try {
      const r = await headUrl(`/admin/products/${id}/image`)
      if (r.status >= 200 && r.status < 300) console.log('IMAGE: OK', `/admin/products/${id}/image`)
      else { console.error('IMAGE: FAIL status', r.status); return process.exit(2) }
    } catch (err) { console.error('IMAGE: FAIL', err.message); console.error(err.stack); return process.exit(2) }
  } else console.log('No products to test image for')
